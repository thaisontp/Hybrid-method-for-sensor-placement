...
for step in range(1, num_sensors):
    ...
    # Tính khoảng cách đến nút gần nhất để in ra log
    closest_dist = min([get_distance(best_next_sensor, s) for s in selected_sensors[:-1]])
    print(f"Cảm biến #{step+1}: Nút {best_next_sensor} (Entropy: {entropy_series[best_next_sensor]:.4f} | Cách nút gần nhất: {closest_dist:.0f})")

print(f"\n=> Danh sách {len(selected_sensors)} cảm biến TOÀN DIỆN trên Net3: {selected_sensors}")

# ==========================================
# 5. PLOT MẠNG LƯỚI NET3 VỚI TÔ HEAT ENTROPY CÁC NODE
# ==========================================
# Tạo node_attribute cho plot
node_attribute = {}
for node in wn.node_name_list:
    if str(node) in normalized_entropy.index:
        node_attribute[node] = normalized_entropy[str(node)]
    else:
        node_attribute[node] = 0  # Hoặc giá trị mặc định cho các node không có entropy

# Plot mạng lưới với heat map dựa trên entropy
fig, ax = plt.subplots(figsize=(12, 8))
wntr.graphics.plot_network(wn, node_attribute=node_attribute, node_cmap='hot', 
                           node_size=50, link_width=1, ax=ax, 
                           title='Net3 Network with Entropy Heat Map')
plt.colorbar(ax.collections[0], ax=ax, label='Normalized Entropy')
plt.savefig('net3_entropy_heatmap.png')
plt.close()
print("\nĐã lưu bản đồ heatmap thành công tại 'net3_entropy_heatmap.png'")